...
rgb_images = np.stack([load_image_rgb(p).numpy() for p in image_paths])
gray_images = np.stack([load_image_gray(p).numpy() for p in image_paths])


dataset = tf.data.Dataset.from_tensor_slices(gray_images)
#dataset = dataset.map(lambda x: (x, x))
dataset = dataset.map(lambda x: (add_noise(x), x))
dataset = dataset.shuffle(buffer_size=len(gray_images), seed=42)

train_size = int(0.8 * len(gray_images))
...
output_img = layers.Conv2D(3, 3, activation="sigmoid", padding="same")(x)
...
all_images_gray = np.stack([load_image_gray(p).numpy() for p in image_paths]) 

encoded_images = encoder.predict(all_images_gray)
...
clustered_images = {i: [] for i in range(22)}

# for idx, label in enumerate(cluster_labels):
#     clustered_images[label].append(all_images[idx])
